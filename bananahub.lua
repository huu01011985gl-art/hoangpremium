-- Banana Hub Premium selective integration
-- Base: BananaCatHubV2
-- Premium-derived areas: UI branding, Leviathan multi-segment logic, Webhook layer, Info
-- All unrelated Fake V2 functionality is preserved.
...
local reportWebhookURL = getgenv().BananaHubWebhookURL or ""
local ideasWebhookURL = getgenv().BananaHubIdeasWebhookURL or ""
...
-- Thêm biến debounce ở đầu file (cùng chỗ với các biến toàn cục khác)
local GrabDebounce = 0

-- Hàm BringEnemy mới
BringEnemy = function(monsterName)
    if not _B then return end
    if tick() - GrabDebounce < 0.3 then return end
    GrabDebounce = tick()
    ...
    pcall(function()
        ...
        -- Lấy tất cả mob cùng tên (nếu monsterName được truyền vào)
        local targetName = monsterName or (PosMon and "any") or nil
        for _, v in pairs(workspace.Enemies:GetChildren()) do
            if v:FindFirstChild("Humanoid") and v.Humanoid.Health > 0 then
                local root = v:FindFirstChild("HumanoidRootPart")
                if root and isnetworkowner(root) then
                    -- Nếu có monsterName, chỉ kéo mob cùng tên; ngược lại kéo mob gần PosMon
                    if monsterName then
                        ...
                        -- Giữ nguyên logic cũ: kéo mob gần PosMon
                        if (root.Position - PosMon).Magnitude <= 300 then
                            table.insert(mobs, v)
                            center = center + root.Position
                            count = count + 1
                        end
        ...
        for _, v in pairs(mobs) do
            ...
            -- Tạo BodyPosition để kéo về tâm
...
local function CanBypassTeleport(targetCF)
    local AreaName = ""
    local worldOrigin = workspace:FindFirstChild("_WorldOrigin")
    if worldOrigin and worldOrigin:FindFirstChild("Locations") then
        for _, loc in pairs(worldOrigin.Locations:GetChildren()) do
            if loc:FindFirstChild("Mesh") and (targetCF.Position - loc.Position).Magnitude <= loc.Mesh.Scale.X then
                AreaName = loc.Name
                break
            end
        end
    ...
    if not getgenv().BypassTP or AreaName:find("Dimension") or AreaName:find("Submerged") or AreaName == "Sealed Cavern" or AreaName:lower():find("under") then
        return false
    end
    if (targetCF.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 3500 then return false end
    return true
end

local function GetBypassCFrame(targetCF)
    local Max = math.huge
    local Pos = nil
    ...
    for _, v in pairs(Spawns) do
        if v:FindFirstChild("Part") then
            local distToTarget = (v.Part.Position - targetCF.Position).Magnitude
            if (targetCF.Position - hrp.Position).Magnitude >= 3000 
                and GetSpawnPoint(v.Part) ~= GetSpawnPoint(hrp)
                and (v.Part.Position - hrp.Position).Magnitude <= 10000
                and distToTarget < Max then
                Max = distToTarget
                Pos = v
            end
        end
...
local function BypassTP(targetCF)
    ...
    if CanBypassTeleport(targetCF) then
        local targetSpawn = GetBypassCFrame(targetCF)
        if targetSpawn and targetSpawn:FindFirstChild("Part") then
            pcall(function()
                if char:FindFirstChild("LastSpawnPoint") then
                    char.LastSpawnPoint.Disabled = true
                end
                replicated.Remotes.CommF_:InvokeServer("SetLastSpawnPoint", targetSpawn.Name)
...
_tp = function(target)
    ...
    local gg = typeof(target) == "CFrame" and target or (typeof(target) == "Vector3" and CFrame.new(target) or (target and target.CFrame))
    ...
    local tweenInfo = TweenInfo["new"](distance / ((getgenv())["TweenSpeed"] or 325), Enum["EasingStyle"]["Linear"])
...
SettingsGay = Window:AddTab("Settings Teleport Bypass")

local BypassGroup = SettingsGay:AddLeftGroupbox("Teleport Bypass")
BypassGroup:AddToggle("BypassTPToggle", {
    Title = "Bypass Teleport (Risk Ban⚠️)",
    Description = "No Reset Tp Collect Fruit",
    Default = false,
...
LeviathanSection:AddToggle("LeviathanMultiSegment", {
    ["Title"] = "Attack Multi Segments Leviathan",
    ["Default"] = false,
    ["Callback"] = function(Value)
        _G["LeviathanMultiSegment"] = Value
    end
})
            ...
            pcall(function()
                for _, leviathan in pairs(workspace["SeaBeasts"]:GetChildren()) do
                    if leviathan["Name"] == "Leviathan" and leviathan:FindFirstChild("HumanoidRootPart") then
                        if getgenv().BananaHubWebhookLeviathan and not getgenv().BananaHubLeviathanWebhookSent then
                            getgenv().BananaHubLeviathanWebhookSent = true
                            BananaHubSendWebhook("Leviathan detected", "Leviathan has been detected in the current server.")
                        end
                        local targets = {}

                        if _G["LeviathanMultiSegment"] then
                            for _, part in pairs(leviathan:GetDescendants()) do
                                if part:IsA("BasePart") and (
                                    part.Name == "Leviathan Segment" or
                                    part.Name == "Leviathan Tail" or
                                    part.Name == "Hitbox11"
                                ) then
                        ...
                        if #targets == 0 then
                            table.insert(targets, leviathan:FindFirstChild("HumanoidRootPart"))
                        end
                        ...
                        for _, target in ipairs(targets) do
                            ...
                            if target and target.Parent then
                                local root = leviathan:FindFirstChild("HumanoidRootPart")
                                if root and (Root.Position - root.Position).Magnitude > 10 then
                                    _tp(root.CFrame * CFrame.new(0, 500, 0))
                                end
                ...
                if not workspace["SeaBeasts"]:FindFirstChild("Leviathan") then
                    getgenv().BananaHubLeviathanWebhookSent = false
                end
            end)
        end
    end
...
-- Premium selective integration: Webhook + Info
local function BananaHubSendWebhook(eventName, detail)
    local url = getgenv().BananaHubWebhookURL
    ...
    local payload = {
        ...
        embeds = {{
            ...
            description = tostring(eventName or "Event"),
            fields = {{
                name = "Detail",
                value = tostring(detail or "Unknown"),
                inline = true
            }, {
                name = "Player",
                value = tostring(Player and Player.Name or "Unknown"),
                inline = true
            }},
            footer = {text = "Banana Hub Premium"},
...
WebhookGroup = WebhookTab:AddLeftGroupbox("Banana Hub Premium Webhook")

WebhookGroup:AddLabel("Set getgenv().BananaHubWebhookURL before execution.")
WebhookGroup:AddToggle("WebhookLeviathan", {
    ["Title"] = "Webhook Find Leviathan",
    ["Default"] = false,
    ["Callback"] = function(Value)
        getgenv().BananaHubWebhookLeviathan = Value
    end
})
WebhookGroup:AddToggle("WebhookFindMirage", {
    ["Title"] = "Webhook Find Mirage",
    ["Default"] = false,
    ["Callback"] = function(Value)
        getgenv().BananaHubWebhookMirage = Value
    end
})
WebhookGroup:AddButton({
    ...
    ["Callback"] = function()
        ...
        if type(req) ~= "function" then
            Library:Notify({
                ["Title"] = "Banana Hub Premium",
                ["Description"] = "HTTP request function is unavailable.",
                ["Duration"] = 3
            })
            return
        end
...
InfoGroup = InfoTab:AddLeftGroupbox("Banana Hub Premium")
InfoGroup:AddLabel("Banana Hub Premium")
InfoGroup:AddLabel("Base: BananaCatHubV2")
InfoGroup:AddLabel("Selective integration: Premium UI / Leviathan / Webhook / Info")
InfoGroup:AddLabel("Fake V2 functionality is preserved unless explicitly replaced.")
InfoGroup:AddButton({
    ["Title"] = "Show Player Info",
    ["Callback"] = function()
        local p = Players.LocalPlayer
        local level = p:FindFirstChild("Data") and p.Data:FindFirstChild("Level")
        Library:Notify({
            ["Title"] = "Banana Hub Premium",
            ["Description"] = string.format("Player: %s | Level: %s | PlaceId: %s", p.Name, level and tostring(level.Value) or "Unknown", tostring(game.PlaceId)),
            ["Duration"] = 5
        })
    end
})
InfoGroup:AddButton({
    ...
    ["Callback"] = function()
        Library:Notify({
            ["Title"] = "Banana Hub Premium",
            ["Description"] = string.format("JobId: %s", tostring(game.JobId)),
            ["Duration"] = 5
        })
    end
})
